...
- a **client**, a program on the user's machine which communicates user's query to the server and receives server's answers. 
- a **server**, a program running in the background that manages access to a specific resource, service or network. The server will understand client's query and apply it to the database. Then it will send an answer to the client.
...
```
... and installed. For the day we are going to choose the version `3.7.7`.
...
To setup your environment just follow this two steps.
